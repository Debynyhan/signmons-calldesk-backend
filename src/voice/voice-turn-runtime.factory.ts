  ...
  buildSteps(): IVoiceTurnStep[] {
    ...
    const preludeStep: IVoiceTurnStep = {
      execute: async (ctx) => {
        ...
        if (prelude.kind === "exit") return { kind: "exit", value: prelude.value };
        return {
          kind: "continue",
          ctx: {
            ...ctx,
    ...
    const contextStep: IVoiceTurnStep = {
      execute: async (ctx) => {
        ...
        if (turnContext.kind === "exit") return { kind: "exit", value: turnContext.value };
        return {
          kind: "continue",
          ctx: {
            ...ctx,
    ...
    const earlyRoutingStep: IVoiceTurnStep = {
      execute: async (ctx) => {
        ...
        if (result.kind === "exit") return { kind: "exit", value: result.value };
        return {
          kind: "continue",
    ...
    const issueCandidateStep: IVoiceTurnStep = {
      execute: async (ctx) => {
        const { tenant, callSid, conversationId, currentEventId, normalizedSpeech, confidence, collectedData } = ctx;
        let nameState = ctx.nameState!;
        let nameReady = ctx.nameReady!;
        const expectedField = ctx.expectedField;

        const existingIssueCandidate = this.deps.voiceTurnPolicyService.getVoiceIssueCandidate(collectedData);
        const issueCandidate = this.deps.voiceTurnPolicyService.normalizeIssueCandidate(normalizedSpeech!);
        const hasIssueCandidate = this.deps.voiceTurnPolicyService.isLikelyIssueCandidate(issueCandidate);
        ...
        if (shouldCaptureOpeningNameFromMultiSlot) {
          ...
          if (hasDeterministicName && deterministicNameCandidate) {
            ...
            nameReady = true;
            const msFirstName = deterministicNameCandidate.split(" ").filter(Boolean)[0] ?? "";
            const msIssueAck = hasIssueCandidate
              ? this.deps.voiceTurnPolicyService.buildIssueAcknowledgement(issueCandidate)
              : null;
            const msTrimmedIssue = msIssueAck?.trim().replace(/[.?!]+$/, "") ?? "";
    ...
    const addressFieldHeuristicStep: IVoiceTurnStep = {
      execute: async (ctx) => {
        ...
        if (!nameReady && !expectedField && this.deps.voiceTurnPolicyService.isLikelyAddressInputForName(normalizedSpeech!)) {
          expectedField = "address";
        }
        const yesNoIntent = this.deps.voiceUtteranceService.resolveBinaryUtterance(normalizedSpeech!);
        if (
          !expectedField &&
          !addressReady &&
    ...
    const turnPlanStep: IVoiceTurnStep = {
      execute: async (ctx) => {
        const { normalizedSpeech, expectedField, nameReady, addressReady, collectedData, existingIssueCandidate, hasIssueCandidate, issueCandidate } = ctx;
        const urgencyConfirmation = this.deps.conversationsService.getVoiceUrgencyConfirmation(collectedData);
        const emergencyIssueContext =
          existingIssueCandidate?.value ?? (hasIssueCandidate ? issueCandidate : "");
        const emergencyRelevant = this.deps.voiceTurnPolicyService.isComfortRiskRelevant(
          existingIssueCandidate?.value ?? (hasIssueCandidate ? issueCandidate! : ""),
        );
        const isQuestionUtterance = this.deps.voiceUtteranceService.isLikelyQuestion(normalizedSpeech!);
        const turnPlan = reduceVoiceTurnPlanner(
          {
            expectedField: expectedField as Parameters<typeof reduceVoiceTurnPlanner>[0]["expectedField"],
            nameReady: nameReady!,
            addressReady: addressReady!,
            issueCaptured: Boolean(existingIssueCandidate?.value || hasIssueCandidate),
            emergencyRelevant: emergencyRelevant!,
    ...
    const nameFlowStep: IVoiceTurnStep = {
      execute: async (ctx) => {
        ...
        const {
          tenant, callSid, conversationId, currentEventId, normalizedSpeech,
          confidence, nameState, csrStrategy, voiceTurnCount, collectedData,
          existingIssueCandidate, timingCollector,
        } = ctx;
        const existingIssueSummary = existingIssueCandidate?.value
          ? this.deps.voiceTurnPolicyService.buildIssueAcknowledgement(existingIssueCandidate.value)
          : null;
        const bookingIntent = this.deps.voiceUtteranceService.isBookingIntent(normalizedSpeech!);
        const isOpeningTurn =
          !expectedField &&
          nameState!.status === "MISSING" &&
          nameState!.attemptCount === 0 &&
    ...
    const expectedFieldStep: IVoiceTurnStep = {
      execute: async (ctx) => {
        const result = await r.turnExpectedFieldRuntime.handleSmsPhoneExpectedField({
          res: ctx.res,
          tenantId: ctx.tenant.id,
          conversationId: ctx.conversationId!,
          callSid: ctx.callSid,
    ...
    const addressNotReadyStep: IVoiceTurnStep = {
      execute: async (ctx) => {
        const { addressReady, expectedField, tenant, conversationId, callSid, displayName, currentEventId, normalizedSpeech, confidence, addressState, nameState, nameReady, collectedData, openingAddressPreface, csrStrategy, timingCollector } = ctx;
        if (addressReady || (expectedField && expectedField !== "address")) {
          return { kind: "continue", ctx };
        }
        const addressPreRoutingResponse = await r.turnAddressRoutingRuntime.handleNotReady({
          res: ctx.res,
          tenantId: tenant.id,
          conversationId: conversationId!,
          callSid: callSid,
    ...
    const addressConfirmedContinuationStep: IVoiceTurnStep = {
      execute: async (ctx) => {
        const { addressState, currentEventId, tenant, conversationId, callSid, displayName, nameState, nameReady, collectedData, csrStrategy, timingCollector } = ctx;
        if (
          addressState!.locked &&
          addressState!.sourceEventId &&
          addressState!.sourceEventId === currentEventId
        ) {
          const result = await r.turnAddressConfirmedRuntime.handleAddressConfirmedContinuation({
            res: ctx.res,
            tenantId: tenant.id,
            conversationId: conversationId!,
            callSid,
